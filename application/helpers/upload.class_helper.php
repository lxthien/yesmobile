...
//        modify by nhthuan
        $file->delete_url = $this->options['script_url'] . 'panel/admin_phone/upload'
                . '?file=' . rawurlencode($file->name);
...
//        $id_cat = $_REQUEST['catid'];
        $CI = get_instance();
        $CI->load->helper('url');        
        $id_cat = $this->_get_category_id_from_referer(current_url());
        $img_arr = array();
...
//            $index_end = strrpos($http_referer, '/');
//            $category_id = substr($http_referer, $index_end + 1);
            $images = $CI->image_model->read_list_by_album_id($id_cat);
            
            if (isset($images)) {
                foreach ($images as $image) {
                    if (isset($image) && strlen($image->name) > 0) {
                        $info = $this->get_file_object($image->name);                        
                        array_push($img_arr, $info);
                    }
//                    else {
//                        $info = $this->get_file_objects();
//                    }
...
//                $CI->image_model->add(array('name' => $item->name, 'album_id' => $category_id));
//            }
        }
...
//            $index_end = strrpos($http_referer, '/');
//            $category_id = substr($http_referer, $index_end + 1);
            $category_id = $this->_get_category_id_from_referer($http_referer);
            if ($category_id === false){
                error_log('Cannot find category id when try to upload image in upload.class_helper.php');
                echo 'Cannot find category id when try to upload image in upload.class_helper.php';
            }            
            
            foreach ($info as $item) {  
                if ($this->get_file_object($item->name)) {
                    $CI->image_model->add(array('name' => $item->name, 'album_id' => $category_id));
                }
            }
        }
...
//         $current .= "delete|".$success."|".$_GET['file']."|\n";
...
//         file_put_contents($file, $current);
    ...
    private function _get_category_id_from_referer($referer){
        if (strlen($referer)===0){
            return false;
        }        
        $index_end = strrpos($referer, '/');
        if ($index_end >=0 && $index_end < strlen($referer)) {
            $category_id = substr($referer, $index_end + 1);            
    ...
    /**
     *This method will be called to delete an image by controller
     * @param type $name 
     */
    public function delete_an_image($file_name){        
        $file_path = $this->options['upload_dir'] . $file_name;
        $success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
        if ($success) {
            foreach ($this->options['image_versions'] as $version => $options) {
